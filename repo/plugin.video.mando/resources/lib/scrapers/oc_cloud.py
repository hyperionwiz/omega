	...
	def _scrape_cloud(self):
		try:
			...
			for item in my_cloud_files:
				...
				if item.get('isDirectory'): append(Thread(target=self._scrape_folders, args=(item['requestId'],)))
				else:
					if not item.get('fileName', '').endswith(tuple(self.extensions)): continue
					match = False
