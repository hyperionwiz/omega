...
# NOTE: this skin copy's "Default" startup-intro option points at
# extras/intro-omega.mp4 (matching this build's naming convention) -
# add that file under extras/ if you want a local fallback to actually
# play; otherwise the fallback attempt is simply a no-op and Kodi
# proceeds straight to Home with no intro, same as "Off".
FALLBACK_INTRO = "special://skin/extras/intro-omega.mp4"
FALLBACK_LABEL = "intro-omega.mp4"
...
def refresh(reason="scheduled"):
    ...
    extras/intro-omega.mp4 if the remote manifest can't be reached, so
    an intro still plays instead of nothing (if that file exists).
